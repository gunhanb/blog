...
```{r, echo=FALSE}
mymean = -15
x <- seq(mymean - 20, mymean + 20, length=100)
hx <- dnorm(x, mymean, 3)
plot(x, hx, type = "l", ylab = "", xlab="Difference of Diastolic blood pressure from baseline to follow-up (mmHg)")
```
...
xmean = 70
ymean = 85
x <- seq(xmean - 20, xmean + 20, length=100)
y <- seq(ymean - 20, ymean + 20, length=100)
hx <- dnorm(x, xmean, 3)
hy <- dnorm(y, ymean, 3)
plot(x, hx, type = "l", ylab = "", 
     xlab="Diastolic blood pressure in control and in treatment (mmHg)", 
     col = "red", xlim = c(50, 100))
lines(y, hx, col = "blue")
legend("topleft", legend = c("Control", "Treatment"), lty = 1, col = c("blue", "red"))
```
